...
Mirrors ``shared/tray/menu_worker.py`` for the PySide6 migration
...
section (see ``qt_menu_manager``).  When the topology is worker-only
...
Pure-Python helpers reuse the shared About dialog from
``qt_about``.  The module does NOT construct a ``QApplication`` and
does NOT touch the legacy pystray modules.
"""
...
class WorkerSection:
    ...
    Constructor signature mirrors the legacy pystray ``WorkerSection``
    in ``shared.tray.menu_worker`` with one addition: the
    ``include_about`` flag lets worker-only topologies expose the

...
PySide6 replacement for :mod:`shared.tray.poller`.  Behavior is
identical to the legacy ``StatePoller`` (2s cadence, 1.5s HTTP
timeout, 3-consecutive-failure threshold, edge-triggered state
transitions and setup-complete notification) — only the delivery
mechanism changes: ``queue.Queue`` enqueues become ``Signal`` emits.

...
"""Unit tests for ``shared/tray/app.py`` context + helper functions.
...
class TestRefreshHelpers:
    ...
    def test_refresh_icon_calls_get_icon_and_set_icon(
        ...
        get_icon = mocker.patch.object(
            app.icons, "get_icon", return_value=pixmap,
        )
        qicon = mocker.patch.object(app, "QIcon")

        app._refresh_icon(ctx, tray)
...
class TestReadManagerPorts:
    ...
    def test_defaults_when_ini_missing(self, tmp_path):
        host, main, lb = app._read_manager_ports(tmp_path)
    ...
    def test_reads_values_from_ini(self, tmp_path):
        ...
        _host, main, lb = app._read_manager_ports(tmp_path)
        assert main == 9000
        assert lb == 9001
...
# ------------------------------------------------------------------ #
# Entrypoint flip — shared.run_tray delegates to app.main
# ------------------------------------------------------------------ #

class TestRunTrayEntrypoint:

    def test_run_tray_calls_app_main(self, mocker):
        import shared.run_tray as rt
        import signal as signal_mod
        mocker.patch.object(signal_mod, "signal")
        spy = mocker.patch.object(app, "main")
        rc = rt.main()
    ...
    def test_run_tray_imports_app(self):
        import shared.run_tray as rt
        # The Phase 8 flip routes through shared.tray.app.

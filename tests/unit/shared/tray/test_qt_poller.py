...
"""Unit tests for ``shared/tray/qt_poller.py`` (FR-15..FR-17a, FR-26).

Port of ``test_poller.py`` for the PySide6-based ``QtStatePoller``.
Queue-draining assertions are replaced with ``QSignalSpy`` counts and
payload inspection.  Shared fixtures live in ``conftest.py``;
thread-lifecycle + cross-thread delivery tests live in
``test_qt_poller_lifecycle.py``.
"""

...
"""Shutdown-sequence tests for ``shared/tray/qt_app.py`` (Phase 8).
...
class TestShutdownOrder:
    ...
    def test_disconnect_uses_static_form_not_instance_method(self, mocker):
        ...
        ctx = _RecordingCtx([])
        spy = mocker.patch.object(qt_app.QObject, "disconnect")
        qt_app._shutdown(ctx)
    ...
    def test_shutdown_noop_when_poller_none(self, mocker):
        ctx = _RecordingCtx([])
        ctx.poller = None
        mocker.patch.object(qt_app.QObject, "disconnect")
        qt_app._shutdown(ctx)  # must not raise
...
class TestShutdownResilience:
    ...
    def test_shutdown_logs_exceptions_via_logger(self, mocker, caplog):
        ...
        ctx = _RecordingCtx([], poller_join_raises=RuntimeError("x"))
        mocker.patch.object(qt_app.QObject, "disconnect")
        with caplog.at_level(logging.ERROR, logger=qt_app.logger.name):
            qt_app._shutdown(ctx)
        assert any(
...
def _patch_main_for_shutdown(mocker, tmp_path,
    ...
    mocker.patch.object(qt_app, "QIcon")
    mocker.patch.object(qt_app, "QTimer")
    mocker.patch.object(qt_app, "qt_icons")
    mocker.patch.object(qt_app, "qt_notifications")
...
class TestMainTryFinally:
    ...
    def test_shutdown_runs_after_exec_returns(
        ...
        shutdown_spy = mocker.patch.object(qt_app, "_shutdown")
    ...
    def test_shutdown_invoked_with_ctx(self, qapp, mocker, tmp_path):
        ...
        assert isinstance(args[0], qt_app._TrayContext)
...
class TestConnectedSlotBehavior:
    ...
    def test_snapshot_slot_refreshes_icon_and_sections(
        ...
        from shared.tray.qt_poller import ManagerSnapshot
        m = _patch_main_for_shutdown(mocker, tmp_path,
                                     topology=topo_mod.TOPOLOGY_BOTH)
        m["app_inst"].exec.return_value = 0
        mocker.patch.object(qt_app, "_shutdown")
        refresh_icon = mocker.patch.object(qt_app, "_refresh_icon")
        refresh_sections = mocker.patch.object(qt_app, "_refresh_sections")
    ...
    def test_notification_slot_dispatches_event(
        ...
        qt_app.qt_notifications.dispatch.assert_called_once_with(
            m["tray_inst"], evt,
        )

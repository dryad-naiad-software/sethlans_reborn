...
"""Unit tests for ``shared/tray/clipboard.py`` (tray Phase 3: FR-4, AC-5).
...
def _reset_qt_message_handler():
    ...
    prior_installed = clipboard._handler_installed
    prior_previous = clipboard._previous_handler
    qInstallMessageHandler(None)
    clipboard._handler_installed = False
    clipboard._previous_handler = None
    try:
        yield
    finally:
        qInstallMessageHandler(None)
        clipboard._handler_installed = prior_installed
        clipboard._previous_handler = prior_previous
...
class TestInputValidation:
    ...
    def test_empty_token_returns_false_no_qt_call(self, mocker):
        fake = mocker.patch.object(clipboard, "QGuiApplication")
    ...
    def test_non_string_types_return_false_no_qt_call(self, mocker, value):
        fake = mocker.patch.object(clipboard, "QGuiApplication")
        assert clipboard.copy_token_to_clipboard(value) is False  # type: ignore[arg-type]
        fake.instance.assert_not_called()

    def test_never_raises_on_bad_input(self):
        try:
            for bad in (None, "", 123, b"bytes"):
                clipboard.copy_token_to_clipboard(bad)  # type: ignore[arg-type]
...
class TestQtLoggingRedaction:
    ...
    def test_handler_installed_after_first_copy(self, qapp):
        assert clipboard._handler_installed is False
        clipboard.copy_token_to_clipboard(_TOKEN)
        assert clipboard._handler_installed is True

    def test_handler_install_is_idempotent(self, qapp, mocker):
        spy = mocker.spy(clipboard, "qInstallMessageHandler")
        clipboard.copy_token_to_clipboard(_TOKEN)
        clipboard.copy_token_to_clipboard(_TOKEN)
        clipboard.copy_token_to_clipboard(_TOKEN)

...
"""Unit tests for ``shared/tray/qt_about.py`` (NFR-1 LGPLv3 attribution).
...
pytest.importorskip("PySide6", reason="PySide6 required for qt_about")
pytest.importorskip("pytestqt", reason="pytest-qt required for qapp fixture")

from PySide6.QtWidgets import QMessageBox  # noqa: E402

from shared.tray import qt_about  # noqa: E402
from shared.tray.qt_about import (  # noqa: E402
    ABOUT_BODY,
    ABOUT_TITLE,
    show_about_dialog,
)
...
class TestShowAboutDialog:
    ...
    def test_exception_is_swallowed(self, qapp, mocker, caplog):
        mocker.patch.object(
            qt_about, "QMessageBox", side_effect=RuntimeError("bad"),
        )
        with caplog.at_level(logging.WARNING, logger=qt_about.logger.name):
            # Must not raise.
            show_about_dialog()
        assert any(
...
class TestShowAboutDialogRealWidget:
    ...
    def test_real_dialog_sets_expected_title(self, qapp, mocker):
        ...
        mocker.patch.object(qt_about, "QMessageBox", side_effect=_factory)
        ...
        qt_about.QMessageBox.Icon = real_cls.Icon
        qt_about.QMessageBox.StandardButton = real_cls.StandardButton
        show_about_dialog()

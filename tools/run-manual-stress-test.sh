...
upload_asset() {
    ...
    if [[ $http_code -ge 200 && $http_code -lt 300 ]]; then
        ...
        echo "  ✅ Asset '$name' uploaded with ID: $asset_id" >&2 # FIX: Redirect progress to stderr
        echo "$asset_id" # This is the actual return value
    else
...
CPU_FALLBACK_PAYLOAD=$(jq -n \
    ...
    --arg project_id "$PROJECT_ID" \
    --argjson asset_id "$ANIMATION_ASSET_ID" \
...
CPU_JOB_PAYLOAD=$(jq -n \
    ...
    --arg project_id "$PROJECT_ID" \
    --argjson asset_id "$SIMPLE_SCENE_ASSET_ID" \
...
CPU_TILED_PAYLOAD=$(jq -n \
    --arg name "CPU-Tiled-High-Samples-$TIMESTAMP" \
    --arg project_id "$PROJECT_ID" \
    --argjson asset_id "$BMW_ASSET_ID" \

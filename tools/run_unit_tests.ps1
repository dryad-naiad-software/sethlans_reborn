...
$ManagerTests = Join-Path $ProjectRoot "manager/tests"
$WorkerTests = Join-Path $ProjectRoot "manager/workers"
$AgentTests = Join-Path $ProjectRoot "worker/tests"

# Build and run pytest command
$command = "pytest -s -v `"$ManagerTests`" `"$AgentTests`" `"$WorkerTests`""

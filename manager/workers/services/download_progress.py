...
"""
Shared download progress tracking for setup wizard background tasks.
...
def find_active_task(prefix: str) -> Optional[tuple[str, DownloadProgress]]:
    """Find an in-progress task whose id starts with *prefix*.

    Used to guard against duplicate download starts.  The ``prefix``
    is a category tag set by the caller (e.g. ``"ffmpeg_"`` or
    ``"blender_"``).
    """
    with _tasks_lock:

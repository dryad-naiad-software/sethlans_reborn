...
from ..constants import (
    TilingConfiguration, RenderEngine, CyclesFeatureSet,
    RenderDevice, VideoStatus,
)
from .upload_paths import (
    animation_frame_output_upload_path, animation_video_upload_path,
    thumbnail_upload_path,
)
from .projects import Project, Asset
from .jobs import JobStatus

_VIDEO_SETTINGS_REQUIRED_KEYS = frozenset(
    {'preset', 'container', 'codec', 'framerate', 'crf'}
)
...
class Animation(models.Model):
    ...
    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, related_name='animations',
    )
    ...
    asset = models.ForeignKey(
        Asset, on_delete=models.CASCADE, related_name='animations',
    )
    output_file_pattern = models.CharField(max_length=1024)
    start_frame = models.IntegerField()
    end_frame = models.IntegerField()
    frame_step = models.IntegerField(
        default=1,
        help_text="Number of frames to advance between renders.",
    )
    status = models.CharField(
        max_length=50, choices=JobStatus.choices,
    ...
    render_engine = models.CharField(
        max_length=50, choices=RenderEngine.choices,
        default=RenderEngine.CYCLES,
    )
    render_device = models.CharField(
        max_length=10, choices=RenderDevice.choices,
        default=RenderDevice.ANY,
    )
    cycles_feature_set = models.CharField(
        max_length=50, choices=CyclesFeatureSet.choices,
        default=CyclesFeatureSet.SUPPORTED,
    )
    render_settings = models.JSONField(
        default=dict, blank=True,
        help_text="Blender render settings overrides.",
    )
    total_render_time_seconds = models.IntegerField(
        default=0,
        help_text="Cumulative render time of all completed frames.",
    )
    tiling_config = models.CharField(
        max_length=10, choices=TilingConfiguration.choices,
    ...
    video_settings = models.JSONField(
        ...
        help_text="Video assembly config: preset, container, codec, etc.",
    )
    video_status = models.CharField(
        max_length=20, choices=VideoStatus.choices,
        null=True, blank=True, default=None,
        help_text="Current state of video assembly.",
    )
    video_file = models.FileField(
        upload_to=animation_video_upload_path,
        null=True, blank=True, max_length=512,
        help_text="The assembled video file.",
    )
    video_error = models.TextField(
        blank=True, default='',
        help_text="Error message from failed video assembly.",
    )
    ...
    def clean(self):
        ...
        if self.video_settings is not None:
            self._validate_video_settings_schema()

    def _validate_video_settings_schema(self):
        if not isinstance(self.video_settings, dict):
            raise ValidationError({
                'video_settings': 'video_settings must be a JSON object.'
            })
        missing = _VIDEO_SETTINGS_REQUIRED_KEYS - set(
            self.video_settings.keys()
        )
        if missing:
            raise ValidationError({
                'video_settings': (
...
class AnimationFrame(models.Model):
    ...
    animation = models.ForeignKey(
        Animation, on_delete=models.CASCADE, related_name='frames',
    )
    frame_number = models.IntegerField()
    status = models.CharField(
        max_length=50, choices=AnimationFrameStatus.choices,
        default=AnimationFrameStatus.PENDING,
    )
    output_file = models.FileField(
        upload_to=animation_frame_output_upload_path,
        null=True, blank=True, max_length=512,
